...
THEORY ListInvariantX IS
  ...
  Abstract_List_Invariant(Refinement(Estacionamento_r3))==(ativo: VAGAS --> BOOL & card(ativo|>{TRUE}) = cv & dom(ativo|>{TRUE}) = dom(tipo) & dom(ativo|>{TRUE}) = dom(status) & cv: 0..MAX_INT & cv = card(tipo) & cv = card(status) & cv<=MAX(comum)+MAX(idoso)+MAX(deficiente) & status: VAGAS +-> STATUS & tipo: VAGAS +-> TIPOS & dom(status) = dom(tipo) & card(tipo|>{comum})<=MAX(comum) & card(tipo|>{idoso})<=MAX(idoso) & card(tipo|>{deficiente})<=MAX(deficiente));
  Context_List_Invariant(Refinement(Estacionamento_r3))==(btrue);
  List_Invariant(Refinement(Estacionamento_r3))==(ids: 1..MAX_INT --> VAGAS & dom(tipo) = ran(ids|>dom(ativo|>{TRUE})) & dom(status) = ran(ids|>dom(ativo|>{TRUE})))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Refinement(Estacionamento_r3))==(cv:=0;tipo:={};status:={};ativo:=VAGAS*{FALSE};@vv.(vv: VAGAS ==> ids:=(1..MAX_INT)*{vv}));
  Context_List_Initialisation(Refinement(Estacionamento_r3))==(skip);
  List_Initialisation(Refinement(Estacionamento_r3))==(cv:=0;tipo:={};status:={};ativo:=VAGAS*{FALSE};ANY vv WHERE vv: VAGAS THEN ids:=(1..MAX_INT)*{vv} END)
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Refinement(Estacionamento_r3),excluir)==(cv>0 & ativo(vv) = TRUE & cv>0 & ativo(vv) = TRUE & cv>0 & vv: VAGAS & vv: dom(tipo) & vv: dom(status) & status(vv) = livre);
  ...
  List_Precondition(Refinement(Estacionamento_r3),indicar)==(vv: VAGAS & tt: TIPOS & card(dom(tipo|>{tt})<|status|>{livre})>0)
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Refinement(Estacionamento_r3),indicar)==(vv: VAGAS & tt: TIPOS & card(dom(tipo|>{tt})<|status|>{livre})>0 | @uu.(uu: dom(dom(tipo|>{tt})<|status|>{livre}) & ativo(uu) = TRUE ==> vv:=uu));
  ...
  Expanded_List_Substitution(Refinement(Estacionamento_r3),excluir)==(cv>0 & ativo(vv) = TRUE & cv>0 & ativo(vv) = TRUE & cv>0 & vv: VAGAS & vv: dom(tipo) & vv: dom(status) & status(vv) = livre | cv:=cv-1;ativo:=ativo<+{vv|->FALSE};tipo:={vv}<<|tipo;status:={vv}<<|status);
  Expanded_List_Substitution(Refinement(Estacionamento_r3),criar)==(cv<MAX_INT & cv<MAX_INT & cv<MAX_INT & tt: TIPOS & card(tipo|>{tt})<MAX(tt) | cv:=cv+1;@vv.(vv: VAGAS & vv/:dom(tipo) & vv/:dom(status) & ativo(vv) = FALSE ==> (ids:=ids<+{cv|->vv};ativo:=ativo<+{vv|->TRUE};tipo:=tipo<+{vv|->tt};status:=status<+{vv|->livre})));
  List_Substitution(Refinement(Estacionamento_r3),criar)==(cv:=cv+1;ANY vv WHERE vv: VAGAS & vv/:dom(tipo) & vv/:dom(status) & ativo(vv) = FALSE THEN ids(cv):=vv;ativo(vv):=TRUE;tipo(vv):=tt;status(vv):=livre END);
...
&
THEORY ListSetsX IS
  Set_Definition(Refinement(Estacionamento_r3),VAGAS)==(?);
  ...
  List_Valuable_Sets(Refinement(Estacionamento_r3))==(VAGAS);
  Inherited_List_Enumerated(Refinement(Estacionamento_r3))==(?);
  Inherited_List_Defered(Refinement(Estacionamento_r3))==(VAGAS);
  Inherited_List_Sets(Refinement(Estacionamento_r3))==(VAGAS);
...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Refinement(Estacionamento_r3))==(VAGAS: FIN(INTEGER) & not(VAGAS = {}));
...
THEORY ListANYVarX IS
  List_ANY_Var(Refinement(Estacionamento_r3),?)==(Var(vv) == atype(VAGAS,?,?));
  List_ANY_Var(Refinement(Estacionamento_r3),criar)==(Var(vv) == atype(VAGAS,?,?));
  ...
  List_ANY_Var(Refinement(Estacionamento_r3),indicar)==(Var(uu) == atype(VAGAS,?,?))
END
&
...
&
THEORY SetsEnvX IS
  Sets(Refinement(Estacionamento_r3)) == (Type(VAGAS) == Cst(SetOf(atype(VAGAS,"[VAGAS","]VAGAS"))))
END
&
THEORY VariablesEnvX IS
  Variables(Refinement(Estacionamento_r3)) == (Type(ids) == Mvl(SetOf(btype(INTEGER,1,MAX_INT)*atype(VAGAS,"[VAGAS","]VAGAS")));Type(ativo) == Mvl(SetOf(atype(VAGAS,"[VAGAS","]VAGAS")*btype(BOOL,0,1)));Type(status) == Mvl(SetOf(atype(VAGAS,?,?)*etype(STATUS,?,?)));Type(tipo) == Mvl(SetOf(atype(VAGAS,?,?)*etype(TIPOS,?,?)));Type(cv) == Mvl(btype(INTEGER,?,?)))
END
&
THEORY OperationsEnvX IS
  Operations(Refinement(Estacionamento_r3)) == (Type(indicar) == Cst(atype(VAGAS,?,?),etype(TIPOS,?,?));Type(get_info_painel) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?),No_type);Type(get_tipo_vaga) == Cst(etype(TIPOS,?,?),atype(VAGAS,?,?));Type(get_cor_lampada) == Cst(etype(CORES,?,?),atype(VAGAS,?,?));Type(liberar) == Cst(No_type,atype(VAGAS,?,?));Type(ocupar) == Cst(No_type,atype(VAGAS,?,?));Type(excluir) == Cst(No_type,atype(VAGAS,?,?));Type(criar) == Cst(No_type,etype(TIPOS,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
