...
THEORY ListVariablesX IS
  ...
  List_Variables(Machine(Controle))==(lim,preco,hora,chegada,pagou,tipo,status,vagas,ini,qtd_max);
  External_List_Variables(Machine(Controle))==(lim,preco,hora,chegada,pagou,tipo,status,vagas,ini,qtd_max)
END
&
...
THEORY ListInvariantX IS
  ...
  Expanded_List_Invariant(Machine(Controle))==(vagas = 1..MAX_INT & status: vagas +-> STATUS_VAGA & tipo: vagas +-> TIPOS & card(tipo|>{comum})<=qtd_max(comum) & card(tipo|>{idoso})<=qtd_max(idoso) & card(tipo|>{deficiente})<=qtd_max(deficiente) & qtd_max: TIPOS --> NAT & ini: BOOL);
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Controle))==(qtd_max,ini:=TIPOS*{0},FALSE;vagas,status,tipo:=1..MAX_INT,{},{};hora,pagou,chegada,preco,lim:=0,TICKET*{nao},{},1,10);
...
THEORY ListOutputX IS
  ...
  List_Output(Machine(Controle),get_tipo_vaga)==(tt);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(Controle),get_tipo_vaga)==(tt <-- get_tipo_vaga(vv));
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Controle),get_tipo_vaga)==(vv: dom(tipo) & tt: TIPOS);
  Own_Precondition(Machine(Controle),get_cor_lampada)==(vv: dom(status) & cc: CORES);
  List_Precondition(Machine(Controle),get_cor_lampada)==(vv: dom(status) & cc: CORES);
  Own_Precondition(Machine(Controle),liberar)==(vv: dom(status) & status(vv) = ocupada);
  List_Precondition(Machine(Controle),liberar)==(vv: dom(status) & status(vv) = ocupada);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Controle),get_tipo_vaga)==(vv: dom(tipo) & tt: TIPOS | tt:=tipo(vv));
  List_Substitution(Machine(Controle),get_cor_lampada)==(IF status(vv) = livre THEN CASE tipo(vv) OF EITHER idoso THEN cc:=azul OR deficiente THEN cc:=amarela OR comum THEN cc:=verde END END ELSE cc:=vermelha END);
  Expanded_List_Substitution(Machine(Controle),get_cor_lampada)==(vv: dom(status) & cc: CORES | status(vv) = livre ==> (not(tipo(vv) = comum) & not(tipo(vv) = deficiente) & tipo(vv) = idoso ==> cc:=azul [] not(tipo(vv) = idoso) & not(tipo(vv) = comum) & tipo(vv) = deficiente ==> cc:=amarela [] not(tipo(vv) = idoso) & not(tipo(vv) = deficiente) & tipo(vv) = comum ==> cc:=verde [] not(tipo(vv) = idoso) & not(tipo(vv) = deficiente) & not(tipo(vv) = comum) ==> skip) [] not(status(vv) = livre) ==> cc:=vermelha);
  List_Substitution(Machine(Controle),liberar)==(status(vv):=livre);
  Expanded_List_Substitution(Machine(Controle),liberar)==(vv: dom(status) & status(vv) = ocupada | status:=status<+{vv|->livre});
  List_Substitution(Machine(Controle),ocupar)==(status(vv):=ocupada);
  Expanded_List_Substitution(Machine(Controle),ocupar)==(vv: dom(status) & status(vv) = livre | status:=status<+{vv|->ocupada});
...
THEORY ListSetsX IS
  ...
  Context_List_Sets(Machine(Controle))==(TIPOS,STATUS_VAGA,CORES,SIM_NAO);
...
THEORY ListPropertiesX IS
  ...
  Context_List_Properties(Machine(Controle))==(MAX_INT: NAT1 & MAX_INT = 1000 & TIPOS: FIN(INTEGER) & not(TIPOS = {}) & STATUS_VAGA: FIN(INTEGER) & not(STATUS_VAGA = {}) & CORES: FIN(INTEGER) & not(CORES = {}) & SIM_NAO: FIN(INTEGER) & not(SIM_NAO = {}));
...
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Machine(Controle),indicar)==(Var(uu) == btype(INTEGER,?,?));
  ...
  List_ANY_Var(Machine(Controle),criar)==(Var(vv) == btype(INTEGER,?,?));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Controle)) == (TICKET | ? | lim,preco,hora,chegada,pagou | V,tipo,status,vagas,V,ini,qtd_max | adiantar,pegar_ticket,pagar_ticket,abrir_cancela | criar,excluir,ocupar,liberar,get_cor_lampada,get_tipo_vaga,get_info_painel,indicar,init | seen(Machine(TiposComuns)),included(Machine(Estacionamento)) | ? | Controle);
  List_Of_HiddenCst_Ids(Machine(Controle)) == (? | ?);
  List_Of_VisibleCst_Ids(Machine(Controle)) == (?);
  List_Of_VisibleVar_Ids(Machine(Controle)) == (? | ?);
  List_Of_Ids_SeenBNU(Machine(Controle)) == (seen(Machine(TiposComuns)): (MAX_INT,TIPOS,STATUS_VAGA,CORES,SIM_NAO,idoso,deficiente,comum,livre,ocupada,azul,amarela,verde,vermelha,sim,nao | ? | ? | ? | ? | ? | ? | ? | ?));
  List_Of_Ids(Machine(Estacionamento)) == (? | ? | tipo,status,vagas | V,ini,qtd_max | criar,excluir,ocupar,liberar,get_cor_lampada,get_tipo_vaga,get_info_painel,indicar | init | seen(Machine(TiposComuns)),included(Machine(QtdMax)) | ? | Estacionamento);
...
THEORY VariablesEnvX IS
  Variables(Machine(Controle)) == (Type(vagas) == Mvl(SetOf(btype(INTEGER,1,MAX_INT)));Type(status) == Mvl(SetOf(btype(INTEGER,?,?)*etype(STATUS_VAGA,?,?)));Type(tipo) == Mvl(SetOf(btype(INTEGER,?,?)*etype(TIPOS,?,?)));Type(ini) == Mvl(btype(BOOL,?,?));Type(qtd_max) == Mvl(SetOf(etype(TIPOS,0,2)*btype(INTEGER,0,MAXINT)));Type(lim) == Mvl(btype(INTEGER,?,?));Type(preco) == Mvl(btype(INTEGER,?,?));Type(hora) == Mvl(btype(INTEGER,?,?));Type(chegada) == Mvl(SetOf(atype(TICKET,"[TICKET","]TICKET")*btype(INTEGER,0,MAXINT)));Type(pagou) == Mvl(SetOf(atype(TICKET,"[TICKET","]TICKET")*etype(SIM_NAO,0,1))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(Controle)) == (Type(criar) == Cst(No_type,etype(TIPOS,?,?));Type(excluir) == Cst(No_type,btype(INTEGER,?,?));Type(ocupar) == Cst(No_type,btype(INTEGER,?,?));Type(liberar) == Cst(No_type,btype(INTEGER,?,?));Type(get_cor_lampada) == Cst(etype(CORES,?,?),btype(INTEGER,?,?));Type(get_tipo_vaga) == Cst(etype(TIPOS,?,?),btype(INTEGER,?,?));Type(get_info_painel) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?),No_type);Type(indicar) == Cst(btype(INTEGER,?,?),etype(TIPOS,?,?));Type(init) == Cst(No_type,btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?));Type(abrir_cancela) == Cst(No_type,atype(TICKET,?,?));Type(pagar_ticket) == Cst(btype(INTEGER,?,?),atype(TICKET,?,?)*btype(INTEGER,?,?));Type(pegar_ticket) == Cst(atype(TICKET,?,?),No_type);Type(adiantar) == Cst(No_type,btype(INTEGER,?,?)));
  Observers(Machine(Controle)) == (Type(get_cor_lampada) == Cst(etype(CORES,?,?),btype(INTEGER,?,?));Type(get_tipo_vaga) == Cst(etype(TIPOS,?,?),btype(INTEGER,?,?));Type(get_info_painel) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?),No_type);Type(indicar) == Cst(btype(INTEGER,?,?),etype(TIPOS,?,?));Type(abrir_cancela) == Cst(No_type,atype(TICKET,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
