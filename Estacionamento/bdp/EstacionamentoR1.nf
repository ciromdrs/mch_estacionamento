...
&
THEORY ListSeesX IS
  List_Sees(Refinement(EstacionamentoR1))==(TiposComuns)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Refinement(EstacionamentoR1))==(criar,excluir,ocupar,liberar,get_cor_lampada,get_tipo_vaga,get_info_painel,indicar);
  List_Operations(Refinement(EstacionamentoR1))==(criar,excluir,ocupar,liberar,get_cor_lampada,get_tipo_vaga,get_info_painel,indicar)
END
&
THEORY ListInputX IS
  ...
  List_Input(Refinement(EstacionamentoR1),get_info_painel)==(?);
...
THEORY ListOutputX IS
  ...
  List_Output(Refinement(EstacionamentoR1),get_info_painel)==(qc,oc,qi,oi,qd,od);
...
THEORY ListHeaderX IS
  ...
  List_Header(Refinement(EstacionamentoR1),get_info_painel)==(qc,oc,qi,oi,qd,od <-- get_info_painel);
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Refinement(EstacionamentoR1),ocupar)==(vv: ativo & vv: dom(status) & status(vv) = livre);
  Own_Precondition(Refinement(EstacionamentoR1),liberar)==(vv: ativo);
  List_Precondition(Refinement(EstacionamentoR1),liberar)==(vv: ativo & vv: dom(status) & status(vv) = ocupada);
  Own_Precondition(Refinement(EstacionamentoR1),get_cor_lampada)==(vv: ativo);
  List_Precondition(Refinement(EstacionamentoR1),get_cor_lampada)==(vv: ativo & vv: dom(status) & vv: dom(tipo) & cc: CORES);
  Own_Precondition(Refinement(EstacionamentoR1),get_tipo_vaga)==(vv: ativo);
  List_Precondition(Refinement(EstacionamentoR1),get_tipo_vaga)==(vv: ativo & vv: dom(tipo) & tt: TIPOS);
  Own_Precondition(Refinement(EstacionamentoR1),get_info_painel)==(btrue);
  List_Precondition(Refinement(EstacionamentoR1),get_info_painel)==(qc: NAT & oc: NAT & qi: NAT & oi: NAT & qd: NAT & od: NAT);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Refinement(EstacionamentoR1),get_info_painel)==(qc: NAT & oc: NAT & qi: NAT & oi: NAT & qd: NAT & od: NAT | qc:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = comum & zz: ativo | 1);oc:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = comum & zz: ativo & zz: dom(status) & status(zz) = ocupada | 1);qi:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = idoso & zz: ativo | 1);oi:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = idoso & zz: ativo & zz: dom(status) & status(zz) = ocupada | 1);qd:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = deficiente & zz: ativo | 1);od:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = deficiente & zz: ativo & zz: dom(status) & status(zz) = ocupada | 1));
  Expanded_List_Substitution(Refinement(EstacionamentoR1),get_tipo_vaga)==(vv: ativo & vv: dom(tipo) & tt: TIPOS | tt:=tipo(vv));
  Expanded_List_Substitution(Refinement(EstacionamentoR1),get_cor_lampada)==(vv: ativo & vv: dom(status) & vv: dom(tipo) & cc: CORES | status(vv) = livre ==> (not(tipo(vv) = deficiente) & tipo(vv) = idoso ==> cc:=azul [] not(tipo(vv) = idoso) & tipo(vv) = deficiente ==> cc:=amarela [] not(tipo(vv) = idoso) & not(tipo(vv) = deficiente) ==> cc:=verde) [] not(status(vv) = livre) ==> cc:=vermelha);
  Expanded_List_Substitution(Refinement(EstacionamentoR1),liberar)==(vv: ativo & vv: dom(status) & status(vv) = ocupada | status:=status<+{vv|->livre});
  Expanded_List_Substitution(Refinement(EstacionamentoR1),ocupar)==(vv: ativo & vv: dom(status) & status(vv) = livre | status:=status<+{vv|->ocupada});
  ...
  List_Substitution(Refinement(EstacionamentoR1),get_info_painel)==(qc:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = comum & zz: ativo | 1);oc:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = comum & zz: ativo & zz: dom(status) & status(zz) = ocupada | 1);qi:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = idoso & zz: ativo | 1);oi:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = idoso & zz: ativo & zz: dom(status) & status(zz) = ocupada | 1);qd:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = deficiente & zz: ativo | 1);od:=SIGMA(zz).(zz: dom(tipo) & tipo(zz) = deficiente & zz: ativo & zz: dom(status) & status(zz) = ocupada | 1));
...
THEORY ListInstanciatedParametersX IS
  List_Instanciated_Parameters(Refinement(EstacionamentoR1),Machine(TiposComuns))==(?)
END
&
...
THEORY ListConstantsX IS
  List_Valuable_Constants(Refinement(EstacionamentoR1))==(?);
  Inherited_List_Constants(Refinement(EstacionamentoR1))==(?);
  List_Constants(Refinement(EstacionamentoR1))==(?)
END
&
THEORY ListSetsX IS
  Set_Definition(Refinement(EstacionamentoR1),VAGA)==(?);
  Context_List_Enumerated(Refinement(EstacionamentoR1))==(TIPOS,STATUS,CORES);
  Context_List_Defered(Refinement(EstacionamentoR1))==(?);
  Context_List_Sets(Refinement(EstacionamentoR1))==(TIPOS,STATUS,CORES);
  List_Valuable_Sets(Refinement(EstacionamentoR1))==(VAGA);
  Inherited_List_Enumerated(Refinement(EstacionamentoR1))==(?);
  Inherited_List_Defered(Refinement(EstacionamentoR1))==(VAGA);
  Inherited_List_Sets(Refinement(EstacionamentoR1))==(VAGA);
...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Refinement(EstacionamentoR1))==(VAGA: FIN(INTEGER) & not(VAGA = {}));
  Context_List_Properties(Refinement(EstacionamentoR1))==(MAX_INT: NAT1 & MAX_INT = 1000 & MAX = {comum|->10,idoso|->5,deficiente|->5,T_NULL|->0} & MAX: TIPOS --> 0..MAX_INT & MAX_INT>=MAX(comum)+MAX(idoso)+MAX(deficiente) & TIPOS: FIN(INTEGER) & not(TIPOS = {}) & STATUS: FIN(INTEGER) & not(STATUS = {}) & CORES: FIN(INTEGER) & not(CORES = {}));
  Inherited_List_Properties(Refinement(EstacionamentoR1))==(btrue);
  List_Properties(Refinement(EstacionamentoR1))==(btrue)
END
&
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Refinement(EstacionamentoR1),Machine(TiposComuns))==(?);
  Seen_Context_List_Enumerated(Refinement(EstacionamentoR1))==(?);
  Seen_Context_List_Invariant(Refinement(EstacionamentoR1))==(btrue);
  Seen_Context_List_Assertions(Refinement(EstacionamentoR1))==(btrue);
  Seen_Context_List_Properties(Refinement(EstacionamentoR1))==(btrue);
  Seen_List_Constraints(Refinement(EstacionamentoR1))==(btrue);
  Seen_List_Operations(Refinement(EstacionamentoR1),Machine(TiposComuns))==(?);
  Seen_Expanded_List_Invariant(Refinement(EstacionamentoR1),Machine(TiposComuns))==(btrue)
END
&
THEORY ListANYVarX IS
  ...
  List_ANY_Var(Refinement(EstacionamentoR1),get_info_painel)==(?);
  List_ANY_Var(Refinement(EstacionamentoR1),indicar)==(Var(uu) == atype(VAGA,?,?))
END
&
THEORY ListOfIdsX IS
  List_Of_Ids(Refinement(EstacionamentoR1)) == (? | ? | ativo,tipo,status | ? | criar,excluir,ocupar,liberar,get_cor_lampada,get_tipo_vaga,get_info_painel,indicar | ? | seen(Machine(TiposComuns)) | ? | EstacionamentoR1);
...
&
THEORY SetsEnvX IS
  Sets(Refinement(EstacionamentoR1)) == (Type(VAGA) == Cst(SetOf(atype(VAGA,"[VAGA","]VAGA"))))
END
&
