...
THEORY ListVariablesX IS
  ...
  List_Variables(Machine(Estacionamento))==(tipo,cor,status,ini,qtd_max);
  External_List_Variables(Machine(Estacionamento))==(tipo,cor,status,ini,qtd_max)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Estacionamento))==(qtd_max,ini:=TIPOS*{0},FALSE;status,cor,tipo:={},{},{});
...
THEORY ListOutputX IS
  ...
  List_Output(Machine(Estacionamento),get_tipo_vaga)==(cc);
...
THEORY ListHeaderX IS
  ...
  List_Header(Machine(Estacionamento),get_tipo_vaga)==(cc <-- get_tipo_vaga(vv));
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Estacionamento),get_tipo_vaga)==(vv: dom(tipo) & cc: TIPOS);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Estacionamento),get_tipo_vaga)==(vv: dom(tipo) & cc: TIPOS | cc:=tipo(vv));
  ...
  List_Substitution(Machine(Estacionamento),get_tipo_vaga)==(cc:=tipo(vv));
...
THEORY ListSetsX IS
  ...
  Context_List_Sets(Machine(Estacionamento))==(VAGAS,TIPOS,STATUS_VAGA,CORES,SIM_NAO);
...
THEORY ListPropertiesX IS
  ...
  Context_List_Properties(Machine(Estacionamento))==(MAX_INT: NAT1 & MAX_INT = 1000000 & VAGAS: FIN(INTEGER) & not(VAGAS = {}) & TIPOS: FIN(INTEGER) & not(TIPOS = {}) & STATUS_VAGA: FIN(INTEGER) & not(STATUS_VAGA = {}) & CORES: FIN(INTEGER) & not(CORES = {}) & SIM_NAO: FIN(INTEGER) & not(SIM_NAO = {}));
...
THEORY VariablesEnvX IS
  Variables(Machine(Estacionamento)) == (Type(qtd_max) == Mvl(SetOf(etype(TIPOS,0,2)*btype(INTEGER,0,MAXINT)));Type(ini) == Mvl(btype(BOOL,?,?));Type(tipo) == Mvl(SetOf(atype(VAGAS,"[VAGAS","]VAGAS")*etype(TIPOS,0,2)));Type(cor) == Mvl(SetOf(atype(VAGAS,"[VAGAS","]VAGAS")*etype(CORES,0,3)));Type(status) == Mvl(SetOf(atype(VAGAS,"[VAGAS","]VAGAS")*etype(STATUS_VAGA,0,1))))
END
&
