...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(Estacionamento))==(tipo,status,vagas);
  List_Variables(Machine(Estacionamento))==(tipo,status,vagas,ini,qtd_max);
  External_List_Variables(Machine(Estacionamento))==(tipo,status,vagas,ini,qtd_max)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Estacionamento))==(vagas = 1..MAX_INT & status: vagas +-> STATUS_VAGA & tipo: vagas +-> TIPOS & card(tipo|>{comum})<=qtd_max(comum) & card(tipo|>{idoso})<=qtd_max(idoso) & card(tipo|>{deficiente})<=qtd_max(deficiente))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(Estacionamento))==(qtd_max,ini:=TIPOS*{0},FALSE;vagas,status,tipo:=1..MAX_INT,{},{});
  Context_List_Initialisation(Machine(Estacionamento))==(skip);
  List_Initialisation(Machine(Estacionamento))==(vagas:=1..MAX_INT || status:={} || tipo:={})
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Estacionamento),get_cor_lampada)==(vv: dom(status) & cc: CORES);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(Estacionamento),get_cor_lampada)==(vv: dom(status) & cc: CORES | status(vv) = livre ==> (not(tipo(vv) = comum) & not(tipo(vv) = deficiente) & tipo(vv) = idoso ==> cc:=azul [] not(tipo(vv) = idoso) & not(tipo(vv) = comum) & tipo(vv) = deficiente ==> cc:=amarela [] not(tipo(vv) = idoso) & not(tipo(vv) = deficiente) & tipo(vv) = comum ==> cc:=verde [] not(tipo(vv) = idoso) & not(tipo(vv) = deficiente) & not(tipo(vv) = comum) ==> skip) [] not(status(vv) = livre) ==> cc:=vermelha);
  ...
  Expanded_List_Substitution(Machine(Estacionamento),criar)==(tt: TIPOS & card(tipo|>{tt})<qtd_max(tt) | @vv.(vv: vagas & vv/:dom(tipo) & vv/:dom(status) ==> tipo,status:=tipo<+{vv|->tt},status<+{vv|->livre}));
...
THEORY VariablesEnvX IS
  Variables(Machine(Estacionamento)) == (Type(qtd_max) == Mvl(SetOf(etype(TIPOS,0,2)*btype(INTEGER,0,MAXINT)));Type(ini) == Mvl(btype(BOOL,?,?));Type(tipo) == Mvl(SetOf(btype(INTEGER,?,?)*etype(TIPOS,?,?)));Type(status) == Mvl(SetOf(btype(INTEGER,?,?)*etype(STATUS_VAGA,?,?)));Type(vagas) == Mvl(SetOf(btype(INTEGER,1,MAX_INT))))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(Estacionamento)) == (Type(init) == Cst(No_type,btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?));Type(indicar) == Cst(btype(INTEGER,?,?),etype(TIPOS,?,?));Type(get_info_painel) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?),No_type);Type(get_tipo_vaga) == Cst(etype(TIPOS,?,?),btype(INTEGER,?,?));Type(get_cor_lampada) == Cst(etype(CORES,?,?),btype(INTEGER,?,?));Type(liberar) == Cst(No_type,btype(INTEGER,?,?));Type(ocupar) == Cst(No_type,btype(INTEGER,?,?));Type(excluir) == Cst(No_type,btype(INTEGER,?,?));Type(criar) == Cst(No_type,etype(TIPOS,?,?)));
