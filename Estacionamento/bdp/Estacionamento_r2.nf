...
THEORY ListInvariantX IS
  ...
  Abstract_List_Invariant(Refinement(Estacionamento_r2))==(cv: 0..MAX_INT & cv = card(tipo) & cv = card(status) & cv<=MAX(comum)+MAX(idoso)+MAX(deficiente) & status: VAGA +-> STATUS & tipo: VAGA +-> TIPOS & dom(status) = dom(tipo) & card(tipo|>{comum})<=MAX(comum) & card(tipo|>{idoso})<=MAX(idoso) & card(tipo|>{deficiente})<=MAX(deficiente));
  Context_List_Invariant(Refinement(Estacionamento_r2))==(btrue);
  List_Invariant(Refinement(Estacionamento_r2))==(ativo: VAGA --> BOOL & card(ativo|>{TRUE}) = cv & dom(ativo|>{TRUE}) = dom(tipo) & dom(ativo|>{TRUE}) = dom(status))
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Refinement(Estacionamento_r2))==(cv:=0;tipo:={};status:={};ativo:=VAGA*{FALSE});
  Context_List_Initialisation(Refinement(Estacionamento_r2))==(skip);
  List_Initialisation(Refinement(Estacionamento_r2))==(cv:=0;tipo:={};status:={};ativo:=VAGA*{FALSE})
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Refinement(Estacionamento_r2),excluir)==(cv>0 & ativo(vv) = TRUE & cv>0 & vv: VAGA & vv: dom(tipo) & vv: dom(status) & status(vv) = livre);
  ...
  List_Precondition(Refinement(Estacionamento_r2),indicar)==(vv: VAGA & tt: TIPOS & card(dom(tipo|>{tt})<|status|>{livre})>0)
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Refinement(Estacionamento_r2),indicar)==(vv: VAGA & tt: TIPOS & card(dom(tipo|>{tt})<|status|>{livre})>0 | @uu.(uu: dom(dom(tipo|>{tt})<|status|>{livre}) & ativo(uu) = TRUE ==> vv:=uu));
  ...
  Expanded_List_Substitution(Refinement(Estacionamento_r2),excluir)==(cv>0 & ativo(vv) = TRUE & cv>0 & vv: VAGA & vv: dom(tipo) & vv: dom(status) & status(vv) = livre | cv:=cv-1;ativo:=ativo<+{vv|->FALSE};tipo:={vv}<<|tipo;status:={vv}<<|status);
  Expanded_List_Substitution(Refinement(Estacionamento_r2),criar)==(cv<MAX_INT & cv<MAX_INT & tt: TIPOS & card(tipo|>{tt})<MAX(tt) | cv:=cv+1;@vv.(vv: VAGA & vv/:dom(tipo) & vv/:dom(status) & ativo(vv) = FALSE ==> (ativo:=ativo<+{vv|->TRUE};tipo:=tipo<+{vv|->tt};status:=status<+{vv|->livre})));
  List_Substitution(Refinement(Estacionamento_r2),criar)==(cv:=cv+1;ANY vv WHERE vv: VAGA & vv/:dom(tipo) & vv/:dom(status) & ativo(vv) = FALSE THEN ativo(vv):=TRUE;tipo(vv):=tt;status(vv):=livre END);
...
&
THEORY ListSetsX IS
  Set_Definition(Refinement(Estacionamento_r2),VAGA)==(?);
  ...
  List_Valuable_Sets(Refinement(Estacionamento_r2))==(VAGA);
  Inherited_List_Enumerated(Refinement(Estacionamento_r2))==(?);
  Inherited_List_Defered(Refinement(Estacionamento_r2))==(VAGA);
  Inherited_List_Sets(Refinement(Estacionamento_r2))==(VAGA);
...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Refinement(Estacionamento_r2))==(VAGA: FIN(INTEGER) & not(VAGA = {}));
...
THEORY ListANYVarX IS
  List_ANY_Var(Refinement(Estacionamento_r2),criar)==(Var(vv) == atype(VAGA,?,?));
  ...
  List_ANY_Var(Refinement(Estacionamento_r2),indicar)==(Var(uu) == atype(VAGA,?,?))
END
&
...
&
THEORY SetsEnvX IS
  Sets(Refinement(Estacionamento_r2)) == (Type(VAGA) == Cst(SetOf(atype(VAGA,"[VAGA","]VAGA"))))
END
&
THEORY VariablesEnvX IS
  Variables(Refinement(Estacionamento_r2)) == (Type(ativo) == Mvl(SetOf(atype(VAGA,"[VAGA","]VAGA")*btype(BOOL,0,1)));Type(status) == Mvl(SetOf(atype(VAGA,?,?)*etype(STATUS,?,?)));Type(tipo) == Mvl(SetOf(atype(VAGA,?,?)*etype(TIPOS,?,?)));Type(cv) == Mvl(btype(INTEGER,?,?)))
END
&
THEORY OperationsEnvX IS
  Operations(Refinement(Estacionamento_r2)) == (Type(indicar) == Cst(atype(VAGA,?,?),etype(TIPOS,?,?));Type(get_info_painel) == Cst(btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?)*btype(INTEGER,?,?),No_type);Type(get_tipo_vaga) == Cst(etype(TIPOS,?,?),atype(VAGA,?,?));Type(get_cor_lampada) == Cst(etype(CORES,?,?),atype(VAGA,?,?));Type(liberar) == Cst(No_type,atype(VAGA,?,?));Type(ocupar) == Cst(No_type,atype(VAGA,?,?));Type(excluir) == Cst(No_type,atype(VAGA,?,?));Type(criar) == Cst(No_type,etype(TIPOS,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
